...
dat <- read_lines("D:/UGR/Introduction to DS/Tarea final/autoMPG6/autoMPG6.dat")
start <- which(dat == "@data") + 1
csv_text <- c("Displacement,Horse_power,Weight,Acceleration,Model_year,Mpg",
              dat[start:length(dat)])

write_lines(csv_text, "autoMPG6.csv")

df <- read_csv("autoMPG6.csv",
               col_types = cols(
...
anyNA(df) # no hay NA values
missmap(df)

names(df) == make.names(names(df)) # Todos nombres de columnas son adecuados
